				...
				<div className="px-4 pb-4 pt-1">
					...
					<div className="mb-3 flex items-center gap-2.5">
						<div className="shrink-0">
							<CopilotIcon size="size-8" aria-hidden="true" />
						</div>
						<div>
...
export default function App() {
	...
	const restartServer = useCallback(async (force = false) => {
		try {
			...
			if (res.status === 409) {
				...
				setRestartConfirm(`Active turns in progress (${ids}). Force restart anyway?`);
				return;
			}
	...
	return (
		...
		<div className="flex flex-col" style={{ height: '100%' }}>
						...
						{showNewGuide ? (
							...
							<div className="chat-scroll" style={{ maxHeight: 'calc(100vh - 12rem)', overflowY: 'auto' }}>
								{guides.map(inst => (
									...
									>
										<GuideScrollIcon size="size-4 shrink-0" style={{ color: 'var(--text-muted)' }} />
			...
			{/* Force-restart confirmation dialog */}
			<ConfirmDialog
				open={restartConfirm !== null}
				title="Force Restart"
				message={restartConfirm ?? ''}
				confirmLabel="Restart"
				onConfirm={() => { setRestartConfirm(null); restartServer(true); }}
				...
				{updateStatus && !updateDismissed && (() => {
					...
					return (
						...
						>
							{/* Icon */}
							{restart
								? <RefreshIcon size="size-4 shrink-0" stroke="var(--success)" />
								: <InfoCircleIcon size="size-4 shrink-0" stroke="var(--primary)" />
							}
